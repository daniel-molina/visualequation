...
from visualequation.symbols.delimiters import *
...
def matrix_type(parent):
    class Dialog(QDialog):
        def __init__(self, parent=None):
            ...
            self.repr_type.setPixmap(QPixmap(os.path.join(
                commons.ICONS_DIR, self.symb._name + ".png")))
    ...
    if ok:
        ...
        latex_code = r'\begin{{' + mtype + r'}}' + row_code * n_rows \
                     + r'\end{{' + mtype + r'}}'
        return Op(myname, latex_code, n_rows * n_cols)
    else:
        return None
...
def cases(parent):
    ...
    if ok:
        ...
        latex_code = r'\begin{{cases}}' + case_code * n_cases \
                     + r'\end{{cases}}'
        return Op(myname, latex_code, n_cases * 2)
    else:
        return None
...
def equation_system(parent):
    ...
    if ok:
        ...
        latex_code = r'\begin{{cases}}' + case_code * n_equations \
                     + r'\end{{cases}}'
        return Op(myname, latex_code, n_equations)
    else:
        return None
...
def aligned(parent):
    ...
    if ok:
        ...
        latex_code = r'\begin{{aligned}}{}' + line_code * n_lines \
                     + r'\end{{aligned}}'
        return Op(myname, latex_code, n_lines + 1)
    else:
        return None
